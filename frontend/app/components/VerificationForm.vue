    ...
    <div v-if="!codeVerified" class="card">
      ...
      <form @submit.prevent="handleSendCode" class="form">
        <div class="form-group">
          ...
          <input id="phone" v-model="phone" type="tel" placeholder="+7 (999) 123-45-67"
            pattern="^(\+7|8)[\s\-]?[\(]?\d{3}[\)]?[\s\-]?\d{3}[\s\-]?\d{2}[\s\-]?\d{2}$" required />
        </div>
    ...
    <div v-if="codeSent && !codeVerified" class="card">
      ...
      <form @submit.prevent="handleVerifyCode" class="form">
        <div class="form-group">
          ...
          <input id="code" v-model="verificationCode" type="text" placeholder="123456" maxlength="4" pattern="[0-9]{4}"
            required />
        </div>
